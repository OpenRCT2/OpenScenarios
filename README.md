...
For those unfamiliar with Github, we recommend sending your scenario's .park file to @Kyphii via Discord DM. Otherwise, contributors may submit scenarios for consideration via pull request, with the desired `.park` files added to the `/submissions` subdirectory. 

Scenario submissions should include, for each included scenario:
...
- Scenarios *must* only contain selected objects (rides, scenery, landscape, etc.) which originate from *RCT2*, *Wacky Worlds*, *Time Twister* or *OpenRCT2 Official* sources. RCT1-exclusive or custom content is not permissible.
...
- A note on RCT2 expansion scenery and vehicles - while these are allowed to be used, note that many team members generally discourage their usage for several reasons, and are likely to be critical of their inclusion in submissions. We recommend using these objects sparingly.